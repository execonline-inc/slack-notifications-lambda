...
export const sendSlackMessages = (slackMessage: SlackMessage): Task<SlackNotifierRequestFailed, SlackNotificationSuccess> => {
  const request = post(slackMessage.slackWebhookUrl)
    .withData({text: slackMessage.message.message, channel: slackMessage.slackChannel, username: slackMessage.slackUser})
